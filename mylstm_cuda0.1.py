...
def res2csv(dataset):
    ...
    fout = open("test_predict%s-%s-%s_%s-%s-%s.csv" % (now.year,now.month,now.day,now.hour,now.minute,now.second), "w")
...
for epoch in range(EPOCH):
    ...
    for step, (b_x, b_y) in enumerate(train_loader):        # gives batch data
        ...
        b_x = Variable(b_x).to(cuda0)
        b_y = Variable(b_y).to(cuda0)
    ...
    # save net parameters
    save_dir = "data/model/rnn_param" + str(epoch + 1) + ".pkl"
    try:
        ...
        print("save rnn model after epoch",epoch)
#trans to cpu
rnn.cpu()
